              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <template v-if="log.action === 'llm_response'">
                    ...
                    <!-- When this is the final answer, show special indicator -->
...
const toolConfig = {
  ...
  'interview_agents': {
    ...
    color: 'green',
    icon: 'users' // Users icon - interview
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // Zap icon - fast search
  },
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "QuestionX:" (English format, current backend format)
      ...
      // If no matches, fall back to "number." format
      if (matches.length === 0) {
        const numPattern = /(?:^|[\r\n]+)(\d+)\.\s+/g
...
const extractFinalContent = (response) => {
  ...
  // Try to find content after Final Answer: (alternative pattern)
