...
# Adding this prefix prevents Agent from calling tools, replying directly with text
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        # The following statuses all indicate preparation is completed:
        ...
        # - completed: Run completed, preparation was done long ago
        ...
        if status in prepared_statuses and config_generated:
            ...
            logger.info(f"Simulation {simulation_id} check result: complete (status={status}, config_generated={config_generated})")
            return True, {
                "status": status,
            ...
            logger.warning(f"Simulation {simulation_id} check result: not complete (status={status}, config_generated={config_generated})")
            return False, {
...
def prepare_simulation():
    ...
    Features:
    - Automatically detect completed preparation to avoid redundant generation
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "message": "Preparation task started | Completed preparation exists",
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "Completed preparation already exists, no need to regenerate",
            ...
            if is_prepared:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "progress": 100,
                        "message": "Completed preparation exists",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
...
def _get_report_id_for_simulation(simulation_id: str) -> str:
    ...
    try:
        ...
        logger.warning(f"Finding simulation {simulation_id} report failed: {e}")
        return None
...
def get_simulation_history():
    ...
    For homepage historical project display, returns simulation list with rich info including project name and description
    
    Query parameters:
        limit: Limit on number of results（default20）
...
def get_simulation_profiles_realtime(simulation_id: str):
    """
    Real-time fetch of simulation Agent profiles (for viewing progress during generation)
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    - Suitable for real-time viewing during generation
    - Returns additional metadata (file modification time, generation status, etc.)
    - Can return partial info even if configuration is not yet fully generated
...
def start_simulation():
    ...
    try:
        ...
        # Smart state handling: allow restart if preparation is complete
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                ...
                # Process does not exist or has ended, reset state to ready
                logger.info(f"Simulation {simulation_id} preparation completed, resetting to ready (previous status: {state.status.value}）")
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": "Graph memory update requires a valid graph_id, please ensure graph has been built"
                }), 400
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be running (in command-waiting mode after completing simulation loop)

    Request (JSON):
        {
            "simulation_id": "sim_xxxx",       // Required, simulation ID
            "agent_id": 0,                     // required, Agent ID
            "prompt": "What do you think about this matter?",  // required, interview prompt
    ...
    try:
        ...
        if not prompt:
            return jsonify({
                "success": False,
                "error": "Please provide prompt (interview prompt)"
            }), 400
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment not running or closed. Please ensure simulation is complete and in command-waiting mode."
            }), 400
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",            // Required, simulation ID
            "prompt": "What is your overall view on this matter?",  // required, interview prompt (same question for all Agents)
    ...
    Returns:
        {
            ...
            "data": {
                ...
                "message": "Environment running, ready to receive Interview commands"
            }
        }
    """
    try:
...
def close_simulation_env():
    ...
    try:
        ...
        logger.error(f"Close environmentFailed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
