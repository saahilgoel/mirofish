...
class OasisProfileGenerator:
    ...
    # Group/institution entity types (need group representative persona generation)
    GROUP_ENTITY_TYPES = [
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user persona generation expert. Generate detailed, realistic personas for public opinion simulation, maximally restoring existing real-world situations. Must return valid JSON format, all string values must not contain unescaped newlines. Use Chinese."
        return base_prompt
...
2. persona: Detailed persona description (2000 characters of plain text), must include:
   ...
   - Stances and views (attitude toward topics, content that may provoke or move them)
...
2. persona: Detailed account profile description (2000 characters of plain text), must include:
   - Institution basic info (formal name, institution type, founding background, main functions)
   ...
   - Institutional memory (important part of persona, introduce this institution's connection to the event and their existing actions and reactions in the event)
