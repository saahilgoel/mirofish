...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        try:
            ...
            process = subprocess.Popen(
                ...
                start_new_session=True,  # Create new process group, ensuring all related processes are terminated on server shutdown
            )
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        """
        Cross-platform terminate process and child processes
        
        Args:
            process: Process to terminate
            simulation_id: Simulation ID (for logging)
            timeout: Timeout for waiting process to exit (seconds)
        """
        if IS_WINDOWS:
            ...
            try:
                ...
                # If not terminated after timeout, force send SIGKILL
    ...
    def get_all_actions(
        ...
        # Read Twitter action files (auto-set platform to twitter based on file path)
        twitter_actions_log = os.path.join(sim_dir, "twitter", "actions.jsonl")
        ...
        # Read Reddit action files (auto-set platform to reddit based on file path)
        reddit_actions_log = os.path.join(sim_dir, "reddit", "actions.jsonl")
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        """
        Clean up simulation run logs (for forcing a restart of simulation)
    ...
    def register_cleanup(cls):
        ...
        # If not in debug mode, this env var does not exist and registration is still needed
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Specify platform (optional)
                ...
                - None: In dual-platform simulation, interview on both platforms and return combined result
    ...
    def close_simulation_env(
        ...
        Send close environment command to simulation, allowing it to gracefully exit waiting mode
        ...
        try:
            ...
            return {
                "success": True,
                "message": "Environment close command sent (response wait timeout, environment may be shutting down)"
            }
