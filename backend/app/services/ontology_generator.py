...
- Each entity is an "account" or "subject" that can post, interact, and disseminate information on social media
...
A. **Fallback types (must include, placed as the last 2 in the list)**:
   - `Person`: Fallback type for any individual. When a person does not belong to other more specific person types, they fall into this category.
...
class OntologyGenerator:
    ...
    def _build_user_message(
        ...
        if len(combined_text) > self.MAX_TEXT_LENGTH_FOR_LLM:
            ...
            combined_text += f"\n\n...(Original text: {original_length} characters, first {self.MAX_TEXT_LENGTH_FOR_LLM} characters extracted for ontology analysis)..."
...
4. All entity types must be real-world subjects that can post on social media, not abstract concepts
