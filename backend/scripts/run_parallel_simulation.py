...
- After simulation completes, enter command-waiting mode instead of closing environment
...
if sys.platform == 'win32':
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        """
        Wrap open() to use UTF-8 encoding by default for text mode
...
# Add filter immediately at module load to ensure it takes effect before camel code runs
...
def disable_oasis_logging():
    ...
    OASIS logs are too verbose (logging each agent observation and action), we use our own action_logger
    """
...
def fetch_new_actions_from_db(
    ...
    """
    Fetch new action records from database and supplement with full context
...
def _enrich_action_context(
    ...
    """
    Supplement action with context info (post content, usernames, etc.)
    ...
    try:
        ...
        # Repost: supplement original post content and author
            ...
            if new_post_id:
                # Repost original_post_id points to original post
                cursor.execute("""
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Close environment immediately after simulation, no command-waiting mode'
    )
    
    args = parser.parse_args()
    
    # Create shutdown event at main() start to ensure entire program can respond to exit signals
    ...
    log_manager.info("=" * 60)
    log_manager.info(f"Simulation loop complete! Total elapsed: {total_elapsed:.1f}seconds")
    
    # Check whether to enter command-waiting mode
